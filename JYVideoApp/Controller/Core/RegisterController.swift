...
class RegisterController: UIViewController {
    private let headerView = AuthHeaderView(title: "注册", subTitle: "新建你的账户")
    private let usernameField = CustomTextField(fieldType: .username)
    private let emailField = CustomTextField(fieldType: .email)
    private let passwordField = CustomTextField(fieldType: .password)
    private let signUpButton = CustomButton(title: "确认", hasBackground: true, fontSize: .big)
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            self.usernameField.widthAnchor.constraint(equalTo: view.widthAnchor, multiplier: 0.95),
            ...
            self.signUpButton.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -35),
            self.signUpButton.centerXAnchor.constraint(equalTo: headerView.centerXAnchor),
            self.signUpButton.heightAnchor.constraint(equalToConstant: 55),
            self.signUpButton.widthAnchor.constraint(equalTo: view.widthAnchor, multiplier: 0.95),
        ])
    }
